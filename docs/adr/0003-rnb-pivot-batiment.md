...
- **BAN écartée comme crosswalk** : `cad_parcelles` ne couvre que 16% des parcelles DVF (vs 52% pour RNB) et n'ajoute que +2,3 pts → la décision « pas d'ingestion BAN nationale » tient (cf. [SOURCES_DONNEES §1.2](../SOURCES_DONNEES.md)). *Note ultérieure* : la BAN départementale est depuis ingérée de façon **ciblée** (contours CP, crosswalk parcelle↔adresse, cascade de l'[ADR 0004](0004-recuperation-non-matchs-dvf-rnb.md)) — l'API ne sert plus que le géocodage de l'adresse cible et le résiduel.
