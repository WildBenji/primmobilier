"""Télécharge le cadastre Etalab (sections + parcelles + bâtiments + lieux-dits) d'un département et le convertit en GeoParquet.

Source : https://cadastre.data.gouv.fr/data/etalab-cadastre/latest/geojson/departements/{dept}/
Produit (idempotent) :
  data/raw/      cadastre-{dept}-{sections,parcelles,batiments,lieux_dits}.json.gz
  data/interim/  cadastre_{sections,parcelles,batiments,lieux_dits}_{dept}.parquet
...
Les lieux-dits (`nom`, `commune`, polygones) sont la seule maille **nommée infra-communale** :
servent à rattacher un bien à sa localité au détail d'une vente (point-dans-polygone).
...
def main(dept: str) -> None:
    ...
    for layer in ("sections", "parcelles", "batiments", "lieux_dits"):
